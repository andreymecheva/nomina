...
$mpdf->WriteHtml('<br><br><br><br><br><div class="jus">El señor '.$model->nom_emp.' identificado con cedula de ciudadanía No. '.number_format($model->cedula, 0, ',', '.').' de '.$model->ciudad.', labora en nuestra compañía desde el '.$model->dia.' de '.$model->mes.' de '.$model->an.', con contrato a término indefinido, desempeñando el cargo de '.$model->cargo.', devengando un salario
mensual de $'.number_format($model->salario, 2, '.', ',').' hasta $'.number_format($model->salario+$model->hasta, 2, '.', ',').' más un auxilio de rodamiento de $'.$model->rodamiento.'.</div>');
